...
printf ${green}"Starting Docker container: mohitsharma44/$distro-docker-ansible."${neutral}"\n"
docker pull mohitsharma44/$distro-docker-ansible:latest
docker run --detach --volume="$PWD":/etc/ansible/roles/role_under_test:rw --name $container_id $opts mohitsharma44/$distro-docker-ansible:latest $init
